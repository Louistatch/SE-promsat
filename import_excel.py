#!/usr/bin/env python
"""
Script d'importation des indicateurs depuis le fichier Excel
"""
import os
import django
import openpyxl
...
def import_indicateurs():
    ...
    # Créer une sous-composante par défaut
    ...
    # Lire les indicateurs (à partir de la ligne 5)
    count = 0
    for row in ws.iter_rows(min_row=5, values_only=True):
        if not row[1]:  # Si pas de code, on arrête
            continue
        ...
        # Déterminer la sous-composante selon le code
        ...
        if created:
            count += 1
            print(f"✅ {code} - {libelle[:50]}...")
    
    print(f"\n🎉 {count} indicateurs importés avec succès!")
    print(f"📊 Total indicateurs dans la base: {Indicateur.objects.count()}")
